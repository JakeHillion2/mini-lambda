...
let rec check_expr scope expr
  ...
  | LogicalAndExpr(loc, lhs, rhs) ->
    let lhs', ty_lhs = check_expr scope lhs in
    unify loc ty_lhs TyBool;
    let rhs', ty_rhs = check_expr scope rhs in
    unify loc ty_rhs TyBool;
    Typed_ast.LogicalAndExpr(loc, lhs', rhs'), TyBool
  | LogicalOrExpr(loc, lhs, rhs) ->
    let lhs', ty_lhs = check_expr scope lhs in
    unify loc ty_lhs TyBool;
    let rhs', ty_rhs = check_expr scope rhs in
    unify loc ty_rhs TyBool;
    Typed_ast.LogicalOrExpr(loc, lhs', rhs'), TyBool
  ...
  = let rec iter (nb, acc) scope stats = match stats with
    ...
    | BindStmt(loc, name, e) :: rest ->
      ...
      let (nb, next_nb, bind_ty, scope') = match scope with
        | BindScope(map) :: rest ->
            if IdentMap.mem name map then
              let id, ty = IdentMap.find name map in (id, nb, ty, scope)
            else
              let ty = new_ty_var () in
        ...
        | other ->
            ...
            (nb, nb + 1, ty, (BindScope map) :: other)
