...
let compile_closure out { id; num_params; num_locals; name; insts; _ } =
  ...
  Array.iter
    ...
    | Closure(i, num_capture) ->
      ...
      Printf.fprintf out "\tpushq %%rcx\n"
    | Label(id, i) ->
      Printf.fprintf out "_lambda_c_%s_%d:\n" id i
    | Jump(id, i) ->
      Printf.fprintf out "\tjmp _lambda_c_%s_%d\n" id i
    | CondJump(id, i) ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\ttest %%rcx, %%rcx\n";
      Printf.fprintf out "\tjne _lambda_c_%s_%d\n" id i
    | InvCondJump(id, i) ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\ttest %%rcx, %%rcx\n";
      Printf.fprintf out "\tje _lambda_c_%s_%d\n" id i
    | Add ->
      Printf.fprintf out "\tpopq %%rcx\n";
    ...
    | Equals ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\txor %%rdx, %%rdx\n";
      Printf.fprintf out "\tcmp %%rcx, (%%rsp)\n";
      Printf.fprintf out "\tsete %%dl\n";
      Printf.fprintf out "\tmov %%rdx, (%%rsp)\n"
    | NotEquals ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\txor %%rdx, %%rdx\n";
      Printf.fprintf out "\tcmp %%rcx, (%%rsp)\n";
      Printf.fprintf out "\tsetne %%dl\n";
      Printf.fprintf out "\tmov %%rdx, (%%rsp)\n"
    | LogicalOr ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\tor %%rcx, (%%rsp)\n"
    | LogicalAnd ->
      Printf.fprintf out "\tpopq %%rcx\n";
      Printf.fprintf out "\tand %%rcx, (%%rsp)\n"
    | Call ->
